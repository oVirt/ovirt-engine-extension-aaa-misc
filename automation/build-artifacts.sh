...
git archive --format=tar HEAD | gzip -9 > ${HOME}/rpmbuild/SOURCES/ovirt-engine-extension-aaa-misc-$VERSION.tar.gz
...
sed \
    ...
    > ovirt-engine-extension-aaa-misc.spec

rpmbuild \
    --define "_topmdir $PWD/rpmbuild" \
    --define "_rpmdir $PWD/rpmbuild" \
    -ba --nodeps ovirt-engine-extension-aaa-misc.spec
